...
def livingwell(messageString):
    ...
    """ 
    if total >=10:
        msg +="\nEverything is coming up Milhouse, with a roll of " +str(total)+"""

        Pick 1.
        • Take +1 forward for the session unless something takes a toll on you.
        • You may turn 1 miss into a weak hit or turn 1 weak hit into a strong hit or a miss (Anyone's roll). The MC will describe how you affect the outcome. 
        • Nothing obvious happens, the MC will make a move in your favour (might be during the session, might be immediate). 
        • Nothing noticable, but something cool might(more than likely) will happen from the psychic malestorm. 
        • Take +2 forward for 3 rolls (declare before roll)
        ```"""
    elif total >=7 and total <10:
        msg+= "\nIt'll do, just a "+str(total)
        msg+="""
        
        Pick 1.
        • Take +1 forward for the next 3 rolls.
        • Take +2 forward on a roll of your choice (must declare before use)
